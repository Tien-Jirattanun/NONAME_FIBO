...
public:
	SimOdomPublisher() : Node("sim_odom_publisher") //, Vx_(0.0), Vy_(0.0), W_(0.0)
	{
		// pub_pos = this->create_publisher<std_msgs::msg::Float64MultiArray>("/forward_position_controller/commands", 10);
		// sub_cmd_vel = this->create_subscription<geometry_msgs::msg::Twist>("cmd_vel", 10, std::bind(&SimOdomPublisher::cmdvel_callback, this, _1));
		timer_ = this->create_wall_timer(500ms, std::bind(&SimOdomPublisher::timer_callback, this));
	}
